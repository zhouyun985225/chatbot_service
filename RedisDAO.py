...
class RedisDAO:
    ...
    def cache_data(self, session_id=None, user_id=None, service_id=None, question=None, coreference=None,
                   intention=None, answer=None):
        if coreference == None:
        ...
        with r.pipeline() as pipe:
            ...
            if len(dialogs) >= TRUEVIEW_CHATBOT_MAX_DIALOG_COUNT:
                del dialogs[0]
            insertObject = {'dialog_number': lastDialogNumber + 1, 'question': question, 'coreference': coreference,
                            'intention': intention, 'answer': answer}
...
if __name__ == "__main__":
    ...
    redisDAO.cache_data(session_id=session_id, user_id=userID, service_id=serviceID, question='什么是洗鼻？',
                        coreference='啥是洗鼻？', intention='tumor',
                        answer='概念：主要是借用一定压力（或吸、或用重力、或用机械压力）将生理盐水送入鼻孔，流经鼻前庭（露在头部外面的部分）、鼻窦、鼻道绕经鼻咽部，或从一侧鼻孔排出，或从口部排出。通过以上路径，借助于生理盐水自身的杀菌作用及水流的冲击力，将鼻腔内己聚集的致病及污湄排出，从而使鼻腔恢复正常的生理环境，恢复鼻腔的自我排毒功能，达到保护鼻腔的目的。适应人群：鼻腔肿瘤化疗后，放疗期间和治疗结束以后，鼻腔手术患者，过敏性鼻炎、鼻窦炎，，长期在粉尘等严重环境中的工作者。目的：鼻腔肿瘤放疗过程中鼻腔、鼻窦和鼻咽颅底不可避免地受到照射后引起充血肿胀，出现与口腔黏膜相似的鼻腔黏膜反应，鼻咽癌患者常有鼻黏膜干燥、鼻塞、鼻腔分泌物增多、黏稠。有效预防鼻腔粘连，以清除鼻咽腔黏膜表面的分泌物，减轻放疗反应，增加癌细胞对放射线的敏感度。')
